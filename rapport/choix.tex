...
Pour vector nous avons utilisés les instruction SIMD (SSE2) pour
effectuer 4 calcul de case à la fois. Pour cela nous avons remplis un
registre de 128bits avec nos 4 cases (sur une meme ligne donc continue
en mémoire virtuelle tout du moins). Ensuite pour divisé par 4 nous
avons déplacer notre registre de 2 bits vers la droite puis nous avons
appliquer un masque 128 bits (11000...11000...110000..11000...) pour
effacer les bits qui se retrouverai sur la mauvaise case.
Enfin pour obtenir le résultat du modulo nous appliquons un masque
128bits {3,3,3,3} sur notre registre de départ.
Au final nous n'avons pas obtenus les performances espérer puisque
la gestion des différents registre prend plus de temps que d'effectuer
4 calculs. Cependant peut être qu'avec des registre de 512bits (AVX)
nous aurions obtenus de meilleurs résultats voir un speed-up > 1.
\medskip
...
que les zones autours ne débordent pas sur elle.
...
workgroup de 16x16. A chaque itération on regarde côté CPU une case
mémoire GPU à 1 si il y a eu un changement. Si c'est le cas on inverse
(comme pour les versions swap CPU) le tableau de lecture du tableau
d'écriture (uniquement les pointeurs) puis on lance l'itération
suivante. Ca n'est clairement pas efficace puisque le CPU est obligé
d' aller lire en mémoire GPU à chaque itération. Pour avoir de
meilleur performance il aurait fallut faire p-itération dans le GPU
avant de vérifier niveau CPU.
